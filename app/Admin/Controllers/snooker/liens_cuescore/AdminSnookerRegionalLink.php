...
use \App\Models\SnookerRegionalLink;
use App\Models\SnookerCalendrierRegional;
use OpenAdmin\Admin\Controllers\AdminController;

class AdminSnookerRegionalLink extends AdminController
{
    /**
     ...
     * @var string
     */
    protected $title = 'Lien CueScore snooker régional';
    ...
    {
        $grid = new Grid(new SnookerRegionalLink());
...
JS);
    ...
    {
        $show = new Show(SnookerRegionalLink::findOrFail($id));
    ...
    {
        $form = new Form(new SnookerRegionalLink());
        ...
        if ($form->isCreating()) {
            ...
            $calendrier = SnookerCalendrierRegional::find(request('calendrier_id'));
            if ($calendrier) {
        ...
        $form->saved(function (Form $form) {
            return redirect('/admin/snooker-calendrier-regionals');
        });
